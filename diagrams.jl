...
function only_tension_fieldlines()
    ...
    pts2 = [(x, 1 - Ks[2]/3 * x^2) for x in LinRange(-0.6, 0.6, 10)]
...
function only_fieldlines()
    ...
    r = Reconnection(0.6, 0.1, fl_stl=S(:stroke=>:blue))
    savefig(r, filename="fl_only.png", height=5000, directory=rpath)
    return draw(r)
end


function new_tension_diagram()
    ...
    μ = 0.6
    Δx = 1.0 / μ
    par = Curve([(x, rs[1] + 0.25*μ^2*x^2/rs[1]) for x in LinRange(-Δx, Δx, 100)])
    par += Curve([(x, rs[2] + 0.25*μ^2*x^2/rs[2]) for x in LinRange(-Δx, Δx, 100)])
    par += Curve([(x, rs[3] + 0.25*μ^2*x^2/rs[3]) for x in LinRange(-Δx, Δx, 100)])
    par += Curve([(x, rs[4] + 0.25*μ^2*x^2/rs[4]) for x in LinRange(-Δx, Δx, 100)])
    par += Curve([(x, rs[5] + 0.25*μ^2*x^2/rs[5]) for x in LinRange(-Δx, Δx, 100)])

    sep = S(:strokeDasharray => 7) * Line([0.0, Δx], [0.0, 1.0])
    sep += S(:strokeDasharray => 7) * Line([0.0, -Δx], [0.0, 1.0])

    crc = Curve([(rs[1]*cos(θ), rs[1]*sin(θ)) for θ in LinRange(0, 2*π, 100)])
    crc += Curve([(rs[2]*cos(θ), rs[2]*sin(θ)) for θ in LinRange(0, 2*π, 100)])
    crc += Curve([(rs[3]*cos(θ), rs[3]*sin(θ)) for θ in LinRange(0, 2*π, 100)])
    crc += Curve([(rs[4]*cos(θ), rs[4]*sin(θ)) for θ in LinRange(0, 2*π, 100)])
    crc += Curve([(rs[5]*cos(θ), rs[5]*sin(θ)) for θ in LinRange(0, 2*π, 100)])

    fig1 = crc + crv 
    fig2 = par + crv + sep
    output = fig1 + T(2*Δx + 0.5, -0.5) * fig2

    savefig(output, filename="new_tension.svg", directory=rpath)
    return draw(output)
end
