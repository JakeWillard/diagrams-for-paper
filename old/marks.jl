...
struct BetterLine <: Mark
    xs :: Vector{Float64}
    ys :: Vector{Float64}
    w :: Float64
    dashes :: Any
end
BetterLine(xs, ys; w=0.01, dashes=Nothing) = BetterLine(xs, ys, w, dashes)


struct BetterCurve <: Mark
    pts :: Vector{Tuple{Float64, Float64}}
end

function ζ(bc::BetterCurve)

    xs = [pt[1] for pt in bc.pts]
    ys = [pt[2] for pt in bc.pts]
    N = length(xs)

    output = Line(xs[1:2], ys[1:2])
    for i=2:N-1
        output += Line(xs[i:i+1], ys[i:i+1])
    end

    return output
end

function ζ(bl::BetterLine)

    xs = LinRange(bl.xs[1], bl.xs[2], 100)
    ys = LinRange(bl.ys[1], bl.ys[2], 100)
    pts = [(xs[i], ys[i]) for i=1:100]

    output = Trail(pts=pts, ws=bl.w)
    if bl.dashes == Nothing
        return output
    else
        return S(:strokeDasharray => bl.dashes) * output
    end
end
...
Separatrix(; μ=1.0, Δy=1.0, color=:black, w=0.01) = Separatrix(μ, Δy, color, w)

function ζ(sep::Separatrix)

    dx = sep.Δy/sep.μ
    ln1 = S(:strokeDasharray => 3, :stroke => sep.color) * BetterLine([-dx, dx], [-sep.Δy, sep.Δy], sep.w)
    ln2 = S(:strokeDasharray => 3, :stroke => sep.color) * BetterLine([dx, -dx], [-sep.Δy, sep.Δy], sep.w)
